...
| 风格四（图片背景） | 风格五（视频背景） |
| :---: | :---: |
| 整卡使用自定义外部图片直链作为背景，描述显示在图片内部底部 | 整卡使用自定义外部视频直链作为背景，描述显示在视频内部底部 |
...
| 手机端风格一 | 手机端风格二 | 手机端风格三 | 手机端风格四/五 |
| :---: | :---: | :---: | :---: |
| ![手机端风格一预览](./image/phone_1.png) | ![手机端风格二预览](./image/phone_2.png) | ![手机端风格三预览](./image/phone_3.png) | 与桌面一致：图片/视频整卡背景 |

> 💡 手机端可独立设置 1/2/3 列布局，并根据卡片密度自动优化复制按钮显示；卡片的毛玻璃效果和程度也可以在后台设置里自定义。每个书签都可以在新增/编辑时**独立选择自己的卡片风格**（默认跟随全局设置）；风格四/五需要先在书签中填写「卡片图片 / 卡片视频」外部直链（书签的新增/编辑/待审核表单中均有对应字段）。
...
| 📺 **在线播放** | 书签填入 `.m3u8` 直链即可一键跳转站内播放器在线播放（需流媒体允许跨域访问） |
...
<!-- changelog:start -->
- 📺 **2026-08-20**：新增站内播放器，书签填入 `.m3u8` / `.m3u` 直链时点击书签即可在线播放
- 🎨 **2026-08-20**：书签支持独立选择卡片风格（默认跟随全局设置），可在新增/编辑书签时自定义
- 🖼️ **2026-08-13**：新增卡片风格四（图片背景）与风格五（视频背景），书签支持填写卡片图片/视频外部直链
